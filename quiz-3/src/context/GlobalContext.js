...
export const GlobalProvider = (props) => {
  ...
  const handleDesc = (params) => {
    var desc = params.slice(0, 80);
    return desc + " ...";
  };

  const handleSize = (params) => {
    let sizeGB = params / 1000;
    return sizeGB + " GB";
  };
  ...
  const handleSubmit = (event) => {
    ...
    if (currentId === -1) {
      axios
        .post("https://backendexample.sanbercloud.com/api/mobile-apps", { name, description, category, release_year, size, price, rating, image_url, is_android_app, is_ios_app })
        .then((res) => {
          console.log(res);
          setFetchStatus(true);
      ...
      axios
        .put(`https://backendexample.sanbercloud.com/api/mobile-apps/${currentId}`, { name, description, category, release_year, size, price, rating, image_url, is_android_app, is_ios_app })
        .then((res) => {
          setFetchStatus(true);
          navigate("/manage-data");
        })
  ...
  const handleDelete = (event) => {
    ...
    axios
      .delete(`https://backendexample.sanbercloud.com/api/mobile-apps/${ID_MOBILE_APPS}`)
      .then((res) => {
        setFetchStatus(true);
      })
      .catch((err) => {});
